...
function App() {
  ...
  const makeMove = (e) => {
    ...
    if (pendingMove.source.length > 0 && pendingMove.target.length) {
      ...
      // if (!host) {
      //   sendingMove.source[0] = 11 - sendingMove.source[0];
      //   sendingMove.source[1] = 4 - sendingMove.source[1];
      //   sendingMove.target[0] = 11 - sendingMove.target[0];
      //   sendingMove.target[1] = 4 - sendingMove.target[1];
      // }
  ...
  const displayBoard = () => {
    const displayedBoard = host ? myBoard : transformBoard(myBoard);

    const firstHalf = displayedBoard.slice(0, 6).map((row, y) => (
      <div
        key={`game_row_${y + 1}`}
        style={{
          display: "flex",
    ...
    const secondHalf = displayedBoard.slice(6).map((row, y) => (
      <div
        key={`game_row_${y + 7}`}
        style={{
          display: "flex",
      ...
      <div style={{ width: "35em" }}>
        ...
        {gamePhase === 2 ? (
          <>
            ...
            <h3>
              Source:{" "}
              {host ? pendingMove.source[0] : 11 - pendingMove.source[0]}
              {host ? pendingMove.source[1] : 4 - pendingMove.source[1]}
            </h3>
            <h3>
              Target:{" "}
              {host ? pendingMove.target[0] : 11 - pendingMove.target[0]}
